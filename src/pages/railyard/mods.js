...
export default function RailyardModsPage() {
  ...
  const filtered = useMemo(() => {
    ...
    sorted.forEach((item) => {
      if (SPOTLIGHT_MOD_SET.has(item.id)) {
      ...
      <main className={sharedStyles.page}>
        ...
        <section className={sharedStyles.hero}>
          ...
          <p className={sharedStyles.heroDescription}>
            {translate({
              ...
              message:
                "Explore community mods, filter by tags, and see what's new.",
            })}
          </p>
        ...
        <section className={sharedStyles.grid}>
          {paginated.map((item) => {
            ...
            const isSpotlighted = SPOTLIGHT_MOD_SET.has(item.id);
